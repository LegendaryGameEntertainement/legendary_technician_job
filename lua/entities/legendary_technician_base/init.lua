...
function ENT:StartUse(ply)
    if RPExtraTeams[ply:Team()].name ~= LEGENDARY_TECHNICIAN_JOBNAME then return end
...
function ENT:EnableProgressBar(ply, enabled)
    net.Start("legendary_technician_repair")
...
function ENT:Think()
    if self.BeingUsed then
        ...
        if self.LootProgress >= 100 then
            ...
            local gainMoney = math.random(LEGENDARY_TECHNICIAN_MIN_REWARD,LEGENDARY_TECHNICIAN_MAX_REWARD)
            DarkRP.notify(self.UsingPlayer, 3, 5, "You repaired the object and got "..gainMoney.."$!")
            self.UsingPlayer:addMoney(gainMoney)
            hook.Run("LegendaryTechnicianRepaired",self.UsingPlayer,self)
            self:CancelUse() 
            return 
        end
    end
